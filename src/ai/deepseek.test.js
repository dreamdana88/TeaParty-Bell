...
console.log("\n=== 测试 23：content 为非字符串类型 → invalid_response ===\n");

{
  // content 是 number → 不应强制 toString，应拒绝
  mockFetch(() =>
    new Response(
      JSON.stringify({
        id: "x",
  ...
  try {
    ...
    assertInstanceOf(err, DeepSeekError, "抛出 DeepSeekError");
    assertEqual(err.code, "invalid_response", "错误码 = invalid_response（非 empty_content）");
    assertIncludes(err.message, "类型异常", "错误信息提示类型异常");
    assertIncludes(err.message, "number", "错误信息包含实际类型");
  }
  afterEach();
}

console.log("\n=== 测试 24：content 仅含空白字符 → empty_content ===\n");
...
console.log("\n=== 测试 26：API Key 缺失时 provider 仍可创建（不抛异常）===\n");
...
console.log("\n=== 测试 29：reasoning_content 不被当作最终 content 返回 ===\n");
