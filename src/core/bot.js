...
// 退出码常量
const EXIT_OK = 0;
const EXIT_RUNTIME = 1;   // 可通过重启恢复的运行期故障
const EXIT_PERMANENT = 78; // 永久性配置/权限故障，systemd 不重启
...
export async function start() {
  ...
  // ---- 4. 初始化 BoostThanks Store（Phase 8）----
  ...
  async function shutdown(signal) {
    ...
    // 停止 Health Monitor（优先）
    try { healthMonitor.stop(); } catch (err) {
      logger.error("Health Monitor 停止时发生异常", { message: err.message });
    }

    // 移除生命周期监听器
    try { if (lifecycleLoggerCleanup) lifecycleLoggerCleanup.destroy(); } catch (err) {
      logger.error("Lifecycle Logger 清理时发生异常", { message: err.message });
    }

    // 清理 Observer
    try { if (observerCleanup) observerCleanup.destroy(); } catch (err) {
    ...
    // 关闭 Outbox
    try { await outbox.close(); } catch (err) {
      logger.error("Outbox 关闭时发生异常", { message: err.message });
    }

    // 销毁 Discord Client
    try { await destroy(); } catch (err) {
