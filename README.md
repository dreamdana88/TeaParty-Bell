...
- Boost 助力自动感谢（第一版核心功能，Phase 3 进行中）
- 社区事件播报（规划中）
- 频道内容总结（规划中）
- AI 聊天交互（规划中）
...
| Phase 3：统一事件层 + 聚合 | 🔄 进行中 | BoostEvent 标准化 + 连续助力聚合，已基于 Phase 2 捕获的真实 Boost 事件结构完成自动测试与回放验证（137 tests）；真实连续助力聚合待后续自然生产事件补充观察，不阻塞后续 Phase |
...
- 已完成的真实单次 Boost 测试中观察到一条独立的 type 8 系统消息
